#simulates basic worldle game, bots will play this to test, uses words from words.txt
import random
...
class WordleGame:
    ...
    def get_feedback(self, guess):
        #default is black (b) (means letter not in target word)
        result = ["b"] * 5 
        target_chars = list(self.target_word)
        guess_chars = list(guess)

        #iterate through to check if letter is in target and in correct position AND has not been used already
        #mark as green (g)
        for i in range(5):
            if guess_chars[i] == target_chars[i]:
                result[i] = "g"
                target_chars[i] = None #mark as is used
                guess_chars[i] = None

        #iterate through to check if letter is in target but not in correct position
        #mark as yellow (y)
        for i in range(5):
            if guess_chars[i] is not None and guess_chars[i] in target_chars:
                result[i] = "y"
                target_index = target_chars.index(guess_chars[i])
                target_chars[target_index] = None  #mark as is used
